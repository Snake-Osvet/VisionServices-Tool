...
avatar_webhook = "https://cdn.discordapp.com/attachments/1228379077954768977/1230223215834435657/animated_2.gif?ex=663289a7&is=662014a7&hm=a445ba15046ad989e54bf85759b02a3be69b665f26bdfa5b2fef59dee09948c7&=&format=webp&quality=lossless&width=810&height=810"
...
def Title(title):
    if sys.platform.startswith("win"):
        "WINDOWS"
        ctypes.windll.kernel32.SetConsoleTitleW(f"VisionServices {version_tool} | {title}")
    elif sys.platform.startswith("linux"):
        "LINUX"
        sys.stdout.write(f"\x1b]2;VisionServices {version_tool} | {title}\x07")
        

def Reset():
    if sys.platform.startswith("win"):
        "WINDOWS"
        file = f'python ./VisionServices.py'
        subprocess.run(file, shell=True)
    elif sys.platform.startswith("linux"):
        "LINUX"
        file = f'python3 ./VisionServices.py'
        subprocess.run(file, shell=True)
...
def BrowserPrivate(site, search_bar=True, title="Navigateur Web"):
    ...
    class WebBrowserApp(QMainWindow):
        def __init__(self, url=None, width=1000, height=300, search_bar=True, title="Navigateur Web"):
            ...
            if url:
                self.load_url()
            logo = "./Img/VisionServices_Icon.ico"
    ...
    def main():
        ...
        """)
        browser = WebBrowserApp(url=site, width=500, height=10, search_bar=search_bar, title=f"VisionServices {version_tool} | {title}")  # Utiliser directement le titre
        browser.show()
        sys.exit(app.exec_())
